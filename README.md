...
| 功能 | 说明 |
|------|------|
| 🎓 **格式合规** | 遵循 GDUPT 本科毕业论文（设计）格式要求 |
| 🇨🇳 **中文排版** | 基于 `XeLaTeX` + `ctex`，支持中英混排 |
| 🧩 **模块化** | 章节独立文件，团队协作 & 大型论文 |
| 📚 **参考文献** | `BibTeX` + `gbt7714`（GB/T 7714-2015 标准） |
...
| 📖 **新手友好** | 修改 `Info.tex` 和 `parts/` 即可开始写作 |
...
| 版本 | 日期 | 作者 | 说明 |
|------|------|------|------|
| **v1.0.0** | 2026-05-10 | LiuyiRigel | 清理废弃/重复的包加载代码，精简类文件结构；完善 `.gitignore`；更新文档与 README；修复多处注释笔误；优化代码可读性与维护性 |
| **v0.0.1 β** | 2021-02-16 | Yicheng Zhang | 初始版本；修正了正文部分奇数页页眉会多出一个阿拉伯数字的章号 |


> 📖 本模板基于以下开源项目的代码构建：[thuthesis](https://github.com/tuna/thuthesis)、[phd-thesis-template](https://github.com/kks32/phd-thesis-template/)、[LZUthesis](https://github.com/szsdk/LZUthesis/)
...
同时，衷心感谢以下个人对本项目的直接或间接贡献：

| 贡献者 | 说明 |
|--------|------|
| **Krishna Kumar** | Cambridge 博士论文模板作者|
| **Ruini Xue** (xueruini@gmail.com) | TUNA / thuthesis 模板作者|
| **Shen Zhou** | LZU 模板作者，中文排版与格式参考的重要来源 |
| **zoho@bbs.ctex.org** | 《一份不太简短的 LaTeX 2ε 介绍》作者|
| **HaoLuan** (HLuan@lincoln.ac.uk) |  |
| **CTeX 社区** ([ctex.org](http://www.ctex.org/)) | 中文 LaTeX 社区生态的持续建设者 |

**特别感谢：**

- **张翼成老师**（`zhangyicheng@gdupt.edu.cn`）—— 本模板的原始作者，创建并维护了 `gduptbe.cls` 类文件，为 GDUPT 学子奠定了 LaTeX 写作的基础。
- **LiuyiRigel**（`oshhhs1@gmail.com`）—— 当前维护者，负责模板的持续优化、文档完善与社区支持。
